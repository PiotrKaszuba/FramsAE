...
import re
path_base = "mods2/mutateDistance/**/*mutDist*"
repr = 'f1'
def gatherScores():
    df = pd.DataFrame()
...
def FDC(df):
    ...
    for key in fitnesses.keys():
        ...
        axes[0].set_ylabel('avg. distance from not worse solutions', fontsize=12)
    ...
    plt.ylabel("fitness distance correlation", fontsize=12)
    plt.xlabel("%s models" % repr, fontsize=12)
    plt.tick_params(
...
plot_f(ax, "Mutation magnitude latent", "Valid mutants", 12, "%s_%s_%s" %(repr, aggregation, measure), legend=False, draw_minor=False)
# print(le.classes_)
...
# plt.show()
plot_f(ax, "Model type", "Validity", 12, "%s centroid validity"%repr, False, False)