...
def frams_mutate(frams_cli, evolution_model : EvolutionModel, individual):
    ...
    for i in range(3):
        individual_temp = evolution_model.mutate(np.array([inner_ind]), times=np.random.randint(1,3), magnitude=0.005*np.random.randint(1,5))

        hidden, cell = splitLatentToHiddenAndCell(np.expand_dims(inner_ind, axis=0))
        pred = evolution_model.decoder.predict([hidden, cell, evolution_model.getOnesMaskForFullPrediction(1)])
        genos = evolution_model.get_genos(pred)
        fram = extract_fram(genos[0])
        valid = frams_cli.isValid(fram)
...
if __name__ == "__main__":
    ...
    hof = tools.HallOfFame(5, similar=np.allclose)
    ...
    pop, log = algorithms.eaSimple(pop, toolbox, cxpb=0.2, mutpb=0.8, ngen=GENERATIONS, stats=stats, halloffame=hof,
                                   verbose=True)
